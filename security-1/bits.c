...
int thirdBits(void) {
  ...
  int second = 0x92;
  int third = 0x24;
  return 0 | (first << 24) | (third << 16) | (second << 8) | first; 
}
/* 
...
int allEvenBits(int x) {
  int mask = 0x55;
  int num = 0 | (mask << 24) | (mask << 16) | (mask << 8) | mask;
  return !(num & ~x);
}
/* 
...
int getByte(int x, int n) {
  int byteMask = 0xff;
  int shift = n << 3;
  return ((byteMask << (shift) & x) >> (shift)) & 0xff;
}
/* 
...
int byteSwap(int x, int n, int m) {
  //create the mask to copy the bytes
  int byteMask = 0xff;

  // the amount of shift that we need for both bytes we want
  int nShift = n << 3;
  int mShift = m << 3;

  //get the bytes that we want to swap
  int nByte = ((byteMask << nShift) & x);
  int mByte = ((byteMask << mShift) & x);

  //we would also like to clear the bits that we copied
  int clear = x & (~nByte) & (~mByte);

  return clear | ((nByte >> nShift & 0xff) << mShift) | ((mByte >> mShift & 0xff) << nShift);
